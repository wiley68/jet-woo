<?php
/**
 * WooCommerce Blocks integration for Jet card gateway.
 *
 * @package jetcredit
 */
...
/**
 * Registers and exposes the card gateway for Checkout Blocks.
 */
final class Jet_Card_Payment_Gateway_Blocks extends AbstractPaymentMethodType {

	/**
	 * Classic gateway instance used by the blocks wrapper.
	 *
	 * @var Jet_Card_Payment_Gateway|null
	 */
	private $gateway;

	/**
	 * Payment method name used by WooCommerce Blocks registry.
	 *
	 * @var string
	 */
	...
	/**
	 * Bootstraps gateway settings and classic gateway instance.
	 *
	 * @return void
	 */
	...
	/**
	 * Determines whether this payment method should be active.
	 *
	 * @return bool
	 */
	...
	/**
	 * Returns frontend script handles for blocks checkout.
	 *
	...
	/**
	 * Provides payment method data passed to the checkout block.
	 *
	...
	public function get_payment_method_data() {
		...
		if ( 1 === $jet_eur && 'EUR' === $jet_currency_code ) {
			$jet_price = number_format( $jet_price * 1.95583, 2, '.', '' );
		} elseif ( 2 <= $jet_eur && 'BGN' === $jet_currency_code ) {
			$jet_price       = number_format( $jet_price / 1.95583, 2, '.', '' );
			$jet_sign        = 'евро';
			$jet_sign_second = 2 === $jet_eur ? 'лв.' : '';
